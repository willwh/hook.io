...
exports.log = function (hook, event, data) {
  ...
  if (this.debug) {
    ...
    if (this.quiet) {
      truncatedData = truncatedData.substr(0, 50) + ' ... ';
    }

    var row_width = 83;

    console.log('Event: '.yellow.bold + pad(event, 30).yellow + ' Type: '.cyan.bold + pad(type, 15).cyan + ' Name: '.magenta.bold + pad(name, 15).magenta + ' Data: '.grey.bold + truncatedData.grey);
