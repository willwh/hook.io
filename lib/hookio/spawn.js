...
exports.spawn = function (hooks, callback) {
  ...
  if(self.autoInstallMissingHooks) {
    self.once('spawn::error', function(err){
      // lets try to install missing deps
      if(err.code === "MISSING_MODULE") {
        self.emit('npm::install', 'hook.io-' + err.hook.type, function(err){
          console.log('sup dog', arguments)
        })
      }
    });
  }
  ...
  function spawnHook (hook, next) {
    ...
    if (local) {
      ...
      // arguments. Otherwise, it treats it as the name of a script to execute
      // with `node`, with the rest of the arguments taken from
      // `options.options`. This supports both.
      //
