...
var reserved = ['hook', 'connection', 'children']; 
...
var Hook = exports.Hook = function (options) {
  ...
  //   self.log(self.name, this.event, data);
  // 
  //   
  //   if (self.remote && reserved.indexOf(parts[0]) === -1 && parts.indexOf(self.name) === -1) {
  //     console.log('---------')
  //     console.dir(parts);
  //     console.dir(self.name);
  //     console.log('---------')
  //     self.remote.message(self.name + '::' + this.event, data);
  //   }
  // });
};
...
Hook.prototype.emit = function (event, data, fromParent) {
  ...
  console.log(this.name + '|fromParent: ' + fromParent);
  console.log(this.name + '|this.event: ' + event);
    
  if (fromParent !== true && this.remote && reserved.indexOf(parts[0]) === -1) {
    console.log(this.name + '|broadcast: ' + event);
    //
    // If it is not a remote emission then broadcast it
    //
    this.remote.message(this.name + '::' + event, data);
  }
  ...
  server = dnode(function (client, conn) {
    this.report = function (name, reported) {
      ...
      function checkName (name, id) {
        ...
        if (self._names.indexOf(attemptedName) === -1) {
          self._names.push(attemptedName);
    ...
    self.onAny(function (data) {
      ...
      //
      if (client.message && ~~reserved.indexOf(parts[0]) && parts[0] !== client.name) {
        client.message(this.event, data);
      }
    });
  });
