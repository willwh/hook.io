...
var Hook = exports.Hook = function (options) {
  ...
  this.onAny(function (data) {
    ...
    if (self.remote && ~~reserved.indexOf(parts[0]) && self.name !== parts[0]
      && ~~self.peers.indexOf(parts[0])) {
...
Hook.prototype.listen = function (options, callback) { 
  ...
  server = dnode(function (client, conn) {
    this.report = function (name, reported) {
      ...
      //
      // Now that we have a confirmed name for the client
      // broadcast to its peers so they are aware of the namespace
      // to prevent circular broadcast.
      //
      self.clients.forEach(function (c) {
        client.addPeer(c.name);
        c.addPeer(client.name);
      });

      //
      // Once the peers are synced add this client to 
      // the set of all clients for this instance. 
      //
...
Hook.prototype.connect = function (options, callback) {
  ...
  client = dnode({
    ...
    removePeer: function (name) {
      //
      // TODO: How should remove peer work?
      //
    }
  });
