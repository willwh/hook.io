...
Hook.prototype.spawn = function (hooks, callback) {
  ...
  function spawnHook (name, next) {
    var hook = 'hook.io-' + name,
        hookBin = path.join(hookName, 'bin', 'hookio-' + name),
        keys;
    
    self.log(self.name, 'spawning child', hookName);
    ...
    if (inProcess) {
      ...
      //
      keys = Object.keys(_hooks[hook].module);
      _hooks[hook].Hook  = _hooks[hook].module[keys[0]];
      _hooks[hook]._hook = new (_hooks[hook].Hook)({ 'name': hook })

      //
      // When the hook has fired the `ready` event then continue.
      //
      _hooks[hook]._hook.once('ready', next);
      _hooks[hook]._hook.connect();
    }
    else {
      try {
      ...
      //
      var child = new forever.Monitor(hookBin, {
        max: 10,
        silent: false,
  ...
  async.forEach(hooks, spawnHook, function (err) {
    return err ? callback(err) : callback(null, 'children ready');
  });
};
