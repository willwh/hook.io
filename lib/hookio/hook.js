...
var Hook = exports.Hook = function (options) {
  ...
  //
  // Note: These binds are used for debugging purposes
  //
  self.on('i.*', function(source, event, data){
    //console.log(self.name.magenta + ' currently getting data on my inputs from: '.green + event.toString().yellow + ' ' + JSON.stringify(data).grey);
    if (typeof event == "object") {
      self.log(self.name, source, event);
    }
    else {
  ...
  self.on('o.*', function(source, event, data){
    //console.log(self.name.magenta + ' currently sending data to my outputs on: '.green + event.toString().yellow + ' ' + JSON.stringify(data).grey);
    if (typeof event == "object") {
      self.log(self.name, source, event);
    }
    else {
    ...
    this.report = function (name, callback) {
      ...
      function checkName(name) {
        ...
        if (self._outputs.indexOf(attemptedName) === -1) {
          ...
          self.log(self.name, 'input connected', attemptedName);
