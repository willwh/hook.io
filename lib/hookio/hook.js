...
var Hook = exports.Hook = function (options) {
  ...
  // Generate a "scaffold" hook with --init.
  // Question: Is this the best place to put it? This functionality is weird
  // because it's the only case where you *wouldn't* want the hook to start.
  if (this.init) {
    ...
    console.log('Welcome to the ' + 'HOOK.IO'.rainbow + ' scaffold tool!');
    prompt.get([{
      name: 'hookname',
      ...
      // Switch for whether the default is to generate or not.
      var def = "n";
      ...
      //
      // If the folder exists, the default behavior is to abort. Otherwise, the
      // default is to write it. You get prompted either way.
      //
      if (path.existsSync("./" + i.hookname)) {
        console.log("/!\\ ".yellow
          + "About to "+"overwrite ".yellow + "./".grey+i.hookname.grey+"!");
      }
      else {
        console.log("About to " + "write ".cyan + "./".grey+i.hookname.grey+"!");
        def = "y";
      }
      prompt.get([{
        name: 'ok',
        message: "/!\\ ".yellow + 'Is this okay? '+yn()
      }], function (err, j) {

        var res = (j.ok[0] || def).toLowerCase();
        ...
        if (res === "y") {
          ...
          introspect.generate(i.hookname);
      ...
      function checkVersion (pkginfo) {
        ...
        if (!self.nocheck && semver.neq(pkginfo.version, self.pkginfo.version)) {
          throw new Error("Conflicting hook versions detected!".red +
                          " \n " + (self.type + " remote.version@"+pkginfo.version+" !== local.version@" + self.pkginfo.version).yellow +
                          " \n You are attempting to connect two diffirent versions of hook.io.".red +
                          " \n Please upgrade your hook.io versions to match.".red +
