...
Hook.prototype.spawn = function (hooks, callback) {
  ...
  function spawnHook (name, next) {
    ...
    if (local) {
      ...
      child.on('start', function spawnOnChildStart(foreverchild, data) {
  ...
  async.forEach(hooks, spawnHook, function (err) {
    if (typeof err === 'Error') {
      self.emit('childrenError');
      callback(err, self, 'children error');
    }
    else {
      self.emit('childrenReady');
      callback(null, self, 'children ready');
    }
    return this;
