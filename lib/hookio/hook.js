...
Hook.prototype.listen = function (options, callback) { 
  ...
  this.server = dnode(function (client, conn) {
    this.report = function (name, reported) {
      ...
      //
      function checkName (name, type, id) {
        var attemptedName = name + '-' + id;
        if (Object.keys(self._names).indexOf(attemptedName) === -1) {
          self._names[attemptedName] = type;
          return attemptedName;
        } 
        
        return checkName(name, type, ++id);
      }
      
      //
      // Update the name on the client accordingly
      //
      client.name = checkName(name, client.type, 0);
      self.emit('client::connected', client.name);
      reported(client.name, client.id);
    };
...
Hook.prototype.spawn = function (hooks, callback) {
  ...
  names = Object.keys(hooks);
  types = {};
  // for every single hook, add their type to the names
  names.forEach(function (name) { types[name] = hooks[name].type });
  ...
  function spawnHook (name, next) {
    var hook = 'hook.io-' + types[name],
        hookBin = path.join(hook, 'bin', types[name]),
        options,
        child,
        keys;
    
    self.emit('hook::spawning', name)
  ...
  self.on('client::connected', function onConnect (data) {
    ...
    if (connections === names.length) {
      self.emit('children::ready', hooks);
      self.un('client::connected', onConnect);
    }
  });
...
//
// ['foo', { name: 'bar', port: 5002 }, { name: 'baz', type: 'wonderhook' }] 
//   ==> { foo: { name: 'foo' }, bar: { name: 'bar', port: 5002 }, baz: { name: 'baz', type: 'wonderhook' }
//
// Omitted values are inherited from THIS hook. (port, host, type)
//
Hook.prototype._spawnOptions = function (hooks) {
  var self = this,
      options = {};
  
  function parse (value, name, all) {
    switch (typeof value) {
      case 'string': 
        name = value;
        options[value] = { name: value, type: value};
        break;
      case 'object':
        if ((!name || typeof name !== 'string') && !value.name) {
          throw new Error('Cannot spawn child hook without name');
        }
        // if it's not a name, then value better hold everything
        if (typeof name !== 'string') {
          name = value.name;
        }
        value.name = value.name || name;
        value.type = value.type || value.name;
        options[name] = value;
        break;
      default: 
...
//    }
//
//    --hook-host localhost --hook-port 5010 --hook-name some-hook-name --hook-type type-of-hook --custom foo
//
