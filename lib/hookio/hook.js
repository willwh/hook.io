...
Hook.prototype.listen = function (options, callback) { 
  ...
  self.toIPs(self['hook-host'], function onResolve (err, hosts) {
    ...
    self.server = dnode(function (client, conn) {
      ...
      this.report = function (name, type, initialEvents, reported) {
        //
        ...
        reporter.events = {};
        self._registerEvents(client.name, initialEvents);
...
Hook.prototype.connect = function (options, callback) {
  ...
  client.connect(dnodeOptions, function (remote, conn) {
    ...
    remote.report(self.name, self.type, self.getStringEvents(), function (newName, newID) {
      self.name = newName;
      self.id   = newID;
...
// The parameters are solely there for recursion, do NOT call it with parameters.
//
Hook.prototype.getStringEvents = function getStringEvents(node, paths, currentPath) {
  if(!node) {
  ...
  Object.keys(node).forEach(function(child) {
    ...
    var tempPath = currentPath.slice(0);
    tempPath.push(child);
    getStringEvents(node[child], paths, tempPath);
  });
