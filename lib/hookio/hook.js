      ...
      if (self.authorize) {
        ...
        this.authorize = function (user, password, cb) {
          self.authorize(user, password, function(err, success){
            if(success) {
              self.emit('authorization::success', { user: user, password: password});
              cb(null, true);
            } else {
              self.emit('authorization::failed', { user: user, password: password});
              cb(null, false);
            }
          })
        };
    ...
    if (self.user && self.password) {
      remote.authorize(self.user, self.password, function (err, success) {
        if (success) {
          ...
          self.emit('authorization::failed', { user: self.user, password: self.password })
        }
      });
    } else {
       handshake();
