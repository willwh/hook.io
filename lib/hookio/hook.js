...
var Hook = exports.Hook = function (options) {
  ...
  //
  EventEmitter.call(self, { delimiter: '::', wildcard: true });
  options = options || {};
...
Hook.prototype._listen = function (callback) {
  ...
  server = dnode(function (client, conn) {
    this.report = function (name, reported) {
      ...
      //
      var name = checkName(name, that.id);
      client.name = name;
      ...
      //
      // Once the broadcast has complete 
      //
      self.clients.push(client);
      
      reported(name, that.id);
    };
    ...
    self.onAny(function (data) {
      ...
      // (i.e. local-only event, 'hook::*', 'connection::*' or 'children::*') and 
