...
Hook.prototype.listen = function (options, callback) { 
  ...
  this.server = dnode(function (client, conn) {
    this.report = function (name, type, reported) {
      ...
      function checkName (name, type, id) {
        ...
        return checkName(name, type, host, id);
      }
      ...
      self._names[client.name].session = self.server.proto.sessions[conn.id];
  ...
  self.on("client::details", function(name, callback) {
    if (self._names[name])
      callback(null, self._names[name]);
    else
      callback(new Error("No hook named "+name+" exists (anymore?)"));
  });
  //
...
Hook.prototype.spawn = function (hooks, callback) {
  ...
  self.on('client::connected', function onConnect (data) {
    ...
    if (connections === hooks.length) {
      ...
      self.on('client::connected', onConnect);
    }
  });
