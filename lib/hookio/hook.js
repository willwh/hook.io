...
// #### @local {boolean} Value indicating if this event is local (i.e. should not be broadcast)
...
//
Hook.prototype.emit = function (event, data, local) {
  ...
  if (local !== true && this.remote && reserved.indexOf(parts[0]) === -1) {
    //
  ...
  server = dnode(function (client, conn) {
    ...
    self.onAny(function (data, remote) {
      var parts = this.event.split('::'),
          event = !remote ? [self.name, this.event].join('::') : this.event;
...
Hook.prototype.spawn = function (hooks, callback) {
  ...
  async.forEach(Object.keys(hooks), spawnHook, function (err) {
    ...
    if (err) {
      return self.emit('error::spawn', err);
    }

    //
    // TODO: FIX: The following block of code to use self.many()
    //
    // I tried using self.many('client::connected', Object.keys(hooks).length, fn)
    // but it was firing too many times
    //
    var x = 0, y = Object.keys(hooks).length;
    self.on('client::connected', function(data){
      x++;
      if(x === y ){
...
Hook.prototype.log = function (hook, event, data) {
  ...
  if (this.debug) {
    var truncatedData;
    if(data.length > 50) {
      truncatedData = data.substr(0, 50) + ' ... ';
    } else {
      truncatedData = data;
    }
