...
var Hook = exports.Hook = function (options) {
  ...
  this.defaults['hook-host']   = options['hook-host']   || 'localhost';
  ...
  //
  // Performs a query on the hook to determine
  //
  function query(params, callback) {
      ...
      if(typeof callback !== "function") {
        return self.emit("error", new Error("When using 'query' event, option 'callback' is mandatory and should be a function"))
      }
      if(name) {
        if (self._names[name]) {
...
Hook.prototype.log = function (hook, event, data) {
  ...
  if (this.debug) {
    ...
    console.log('Event: '.yellow.bold + pad(event, 30).yellow + ' Type: '.cyan.bold + pad(type, 15).cyan + ' Name: '.magenta.bold + pad(name, 15).magenta + ' Data: '.grey.bold + data.grey);
