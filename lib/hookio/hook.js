...
var Hook = exports.Hook = function (options) {
  ...
  //
  // The default name for a hook is it's type
  //
  this.defaults.name = this.type;

  //
  // Unless an actual name has been passed in
  //
  this.name = this.name || options.name || options['hook-name'] || this.defaults.name;

  //
  // Logging options for winston
  //
  // Multiple transports can be specified as follows:
  //
  //  (in config.json)
  //  "logger" : {
  //    "transports" : {
  //      "file" : {
  //        "filename" : "/path/to/somefile.log",
  //      }
  //    },
  //
  //    "exceptionHandlers" : {
  ...
  //  }
  //
  //  through Hook instantiation options, i.e. new Hook(options)
  //
  //  {
  //    logger : {
  //      transports : [
  ...
  //  }
  //
  this.logger = this.logger || options.logger || options['hook-logger'];

  //
  // first call to log() will setup transports for winston (if applicable)
  // and return the actual logging method
  //
  this.log    = this.log();
  ...
  //
  // hookBin determines the forever shim to use when spawning child hooks
  //
  this.hookBin = this.hookBin || options.hookBin || __dirname + '/../../bin/forever-shim';
  ...
  //
  // The "autoheal" flag indicates whether or not the hook.io mesh will,
  // attempt to automatically recover from "master" hook failures
  //
  // For more information see: http://blog.nodejitsu.com/fault-tolerant-applications-in-nodejs
  //
  this.autoheal = this.autoheal || options.autoheal || false;
  ...
  //
  // Setup some defaults for hook authorization
  //
  this.authorize = this.authorize || options.authorize || false;
  this.user      = this.user      || options.user      || null;
  this.password  = this.password  || options.password  || null;

  //
  // Automatically install any hooks we attempt to spawn that are not available on this system
  //
  this.autoInstallMissingHooks = this.autoInstallMissingHooks || options.autoInstallMissingHooks || false;
  ...
  if (!this.ignoreSTDIN && !this.repl && !process.listening && !isWorker) {
    ...
    process.stdin.on('data', function (chunk) {
      ...
      lines.forEach(function(line){
        if (line) {
          self.emit('hook::STDIN', line);
          //
          // Let's just try to evaluate all STDIN.
          // Feel free to make this more secure.
          //
          try {
            eval(line);
          } catch (err) {
            //
            // Silently fail STDIN evaluation errors
            //
          }
        }
      });
  ...
  self.on('hook::started', function () {
    ...
    if (self.hooks && self.hooks.length) {
      self.once('hook::children::ready', function () {
...
Hook.prototype.listen = function (options, callback) {
  ...
  this.webserver = options.server || options.webserver || http.createServer();
  ...
  self.toIPs(self['hook-host'], function onResolve (err, hosts) {
    ...
    if (err) {
      ...
      return self.emit('hook::error::resolve', err);
    }
    ...
    self.server = dnode(function (client, conn) {
        ...
        for (name in self._names) {
          if (self._names[name].session === conn.id) {
            self.emit('hook::disconnected', {
              name: self.name,
              proposedRank: self.proposedRank,
              disconnected: { name: name, type: self._names[name].type }
            });
      ...
      if (self.authorize) {
        ...
        this.authorize = function (user, password, cb) {
          self.authorize(user, password, function(err, success){
            if(success) {
              self.emit('hook::authorization::success', { user: user, password: password});
              cb(null, true);
            } else {
              self.emit('hook::authorization::failed', { user: user, password: password});
              cb(null, false);
            }
          })
        };
      ...
      function message (event, data, callback, sender) {
      ...
      function hasEvent (parts, callback) {
    ...
    self.server.on('connection', function (conn) {
      self.emit('hook::connection::open', conn);
    });
    ...
    self.server.on('ready', function () {
      ...
      if (self.autoheal) {
        self.emit('hook::autoheal', 'hook mesh will attempt to recover from failures');
      }
    ...
    self.webserver.on('error', self.server.emit.bind(self.server, 'error'));
    ...
    //
    try {
      self.webserver.listen(self['hook-port'], self['hook-host'], function () {
        self.server.listen(self.webserver);
        self.server.emit('ready');
      });
    }
    catch (ex) {
      if (callback) {
...
Hook.prototype.connect = function (options, callback) {
  ...
  client.on('error', function (data) {
    self.emit('hook::connection::error', data);
  ...
  // TODO: dnodeOptions includes our socket.io stream
  client.connect(dnodeOptions, function (remote, conn) {
    ...
    if (remote.authorize && (!self.user || !self.password)) {
      console.log('Server requires authorization. Try again with remote.authorize(user, pass)');
      return;
    }

    if (self.user && self.password) {
      remote.authorize(self.user, self.password, function (err, success) {
        if (success) {
          handshake();
        } else {
          self.emit('hook::authorization::failed', { user: self.user, password: self.password })
        }
      });
    } else {
       handshake();
    ...
    conn.on('end', function () {
      self.emit('hook::connection::end', { name: self.name, proposedRank: self.proposedRank });
...
Hook.prototype._cliOptions = function (options) {
  ...
  //
  var reserved_cli = ['port', 'host', 'name', 'type', 'src'];
...
Hook.prototype._dnodeOptions = function () {
  ...
  return {
    stream:      http.createConnection(this['hook-port'], this['hook-host']),
    path:        this.socket,
    key:         this.key,
    block:       this.block,
    reconnect:   this.reconnect
